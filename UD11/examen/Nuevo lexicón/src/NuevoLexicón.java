...
public class NuevoLexicón {
  public static void main(String[] args) throws Exception {
    if (args.length != 1) {
      System.out.println("Error: Debe introducir un único argumento.");
    } else {
      String nombreFichero = "./src/lexicón.txt";
      ...
      do {
        ...
        switch (elección) {
          ...
          case 3: // Añadir entrada a diccionario
            ...
            if (miGestor.añadirEntrada(palabraInglés, significadoEspañol))
              System.out.println("Entrada añadida con éxito.");
            else
              System.out.println("No se pudo añadir la entrada");
            break;

          case 4: // Borrar entrada completa del diccionario
            System.out.print("Escriba la palabra en inglés cuya entrada se ha de borrar: ");
            palabraInglés = s.nextLine();
            if (miGestor.eliminarEntrada(palabraInglés))
              System.out.println("Entrada borrada con éxito.");
            else
              System.out.println("No se pudo borrar la entrada");
            break;
          ...
          case 5: // Borrar significado en castellano del diccionario
            ...
            significadoEspañol = s.nextLine();
            if (miGestor.eliminarSignificado(palabraInglés, significadoEspañol))
              System.out.println("Acepción eliminada con éxito.");
            else
              System.out.println("No se pudo eliminar la acepción");
            break;

          case 6: // Importar desde archivo
            if (miGestor.importar(nombreFichero))
              System.out.println("Importación desde fichero realizada con éxito.");
            else
              System.out.println("No se pudo realizar la importación");
            break;

          case 7: // Exportar desde archivo
            if (miGestor.exportar(nombreFichero))
              System.out.println("Exportación a fichero realizada con éxito.");
            else
              System.out.println("No se pudo realizar la exportación");
            break;

          case 8: // Embellecer entradas del archivo
            if (miGestor.embellecer(nombreFichero))
              System.out.println("Embellecimiento de las entradas del fichero realizada con éxito.");
            else
              System.out.println("No se pudo realizar el embellecimiento de las entradas del fichero");
            break;
        }
      } while (elección != 9);
      s.close();
