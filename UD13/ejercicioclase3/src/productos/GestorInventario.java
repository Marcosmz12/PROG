...
public class GestorInventario implements CRUD<Producto>{
    ...
    */
    ...
    public boolean exportar(String baseDatos) {
        ArrayList<Producto> result = new ArrayList<Producto>();
        String sqlQery = "SELECT * FROM productos";
        try (Statement statement = this.conn.createStatement()) {
            ResultSet qResultSet = statement.executeQuery(sqlQery);

            while (qResultSet.next()) {
                long codigo = qResultSet.getLong("codigo");
                String descripcion = qResultSet.getString("descripcion");
                double precioCompra = qResultSet.getDouble("precioCompra");
                double precioVenta = qResultSet.getDouble("precioVenta");
                int stock = qResultSet.getInt("stock");
                result.add(new Producto(codigo, descripcion, precioCompra, precioVenta, stock));
            }
